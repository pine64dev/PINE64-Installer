...
if [ "$ARGV_ARCHITECTURE" == "x64"  ]; then
  ...
  APPIMAGETOOL_CHECKSUM=3475d0fcf43ba08eb483fd413b44476e2f8274196d0d2b482c4bf780a2c30091
elif [ "$ARGV_ARCHITECTURE" == "x86"  ]; then
  APPIMAGES_ARCHITECTURE="i686"
  APPIMAGETOOL_CHECKSUM=812c505a61e493a37b726ace83156a6562d90a8dd2be5088b07ab327f6098bc3
else
...
APPIMAGETOOL_PATH=$ARGV_DOWNLOAD_DIRECTORY/appimagetool-$APPIMAGES_ARCHITECTURE.AppImage
mkdir -p "$ARGV_DOWNLOAD_DIRECTORY"
./scripts/build/download-tool.sh -x \
  -u "$APPIMAGES_GITHUB_RELEASE_BASE_URL/appimagetool-$APPIMAGES_ARCHITECTURE.AppImage" \
  -c "$APPIMAGETOOL_CHECKSUM" \
  -o "$APPIMAGETOOL_PATH"
