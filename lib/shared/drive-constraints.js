...
/**
 * @summary The default size for things such as images and drives
 * @constant
 * @private
 * @type {Number}
 */
...
exports.isDriveLargeEnough = (drive, image) => {
  return _.get(drive, 'size', DEFAULT_SIZE) >= _.get(image, 'size', DEFAULT_SIZE);
};
...
exports.isDriveSizeRecommended = (drive, image) => {
  return _.get(drive, 'size', DEFAULT_SIZE) >= _.get(image, 'recommendedDriveSize', DEFAULT_SIZE);
};
