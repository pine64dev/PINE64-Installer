...
exports.downloadFromMTDFile = (path, progress) => {
  ...
  const task = (resolve, reject) => {
    ...
    }, rejectTask, undefined);

    // eslint-disable-next-line new-cap
    mtd.FinalizeDownload({
      fd$: fdR$,
      meta$: meta$.last()
    }).subscribe(undefined, rejectTask, resolveTask);
  };
...
exports.download = (url, path, progress) => {
  ...
  const mtdExists = () => {
    return startDownload().catch((error) => {
      if (error !== errors.ConnInterruptError) {
        return createDownload().then(startDownload);
      }
      return Bluebird.reject(error);
    });
  };
  ...
  return fs.statAsync(mtdPath).then(mtdExists, mtdMissing);
};
