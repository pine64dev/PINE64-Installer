...
exports.download = (url, path, progress) => {
  ...
  // add delay so that progress of 100% is visible to user.
  const RESOLVE_DELAY = 1500;

  // Download content into mtd file, and catch connection error if raised.
  const startDownload = () => {
  ...
  // Create mtd file, responsible to delete any partially created mtd file.
  ...
  // Create a new mtd file, and download content into it.
    ...
    return createDownload()
      .then(() => {
        return startDownload().then(
          () => {
            progress.complete();
            return Bluebird.delay(RESOLVE_DELAY);
          },
          (error) => {
            progress.abort();
            return Bluebird.reject(error);
  ...
  // Resume mtd file download, will create a new mtd file if failed to resume download on existing one.
