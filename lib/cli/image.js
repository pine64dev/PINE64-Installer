...
exports.fastDownloadImage = (imageUrl, imagePath, progress) => {
  return new Bluebird((resolve, reject) => {
    const MAX_CONCURRENT_CONNECTIONS = 7;
    const MAX_CONNECTION_TIMEOUT = 30000;
    const SIZE_UPDATE_INTERVAL = 500;

    const dl = new FastDownload(imageUrl, {
      destFile: imagePath,
      resumeFile: true,
      chunksAtOnce: MAX_CONCURRENT_CONNECTIONS,
    ...
    const updateDataSize = () => {
      progress.update(dl.downloaded - downloaded);
      downloaded = dl.downloaded;
    };

    dl.on('start', () => {
      progress.setTotalSize(dl.file_size);
      progress.setStartSize(dl.file_size - dl.size);
      progress.startUpdate();
      timeout = setInterval(updateDataSize, SIZE_UPDATE_INTERVAL);
    });
