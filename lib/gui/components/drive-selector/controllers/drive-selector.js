...
module.exports = function(
  ...
  const shouldChangeDriveSelectionState = (drive) => {
    if (!DriveConstraintsModel.isDriveValid(drive, SelectionStateModel.getImage(), this.state.hasOS())) {
  ...
  this.determineIfDriveLargeEnough = (drive) => {
    if (this.state.hasOS()) {
      if (drive.recommendedImage) {
        return this.constraints.isDriveLargeEnough(drive, {
    ...
    return this.constraints.isDriveLargeEnough(drive, this.state.getImage());
  };
};
