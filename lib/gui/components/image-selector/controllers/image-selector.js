...
module.exports = function(
  ...
  this.setCurrentOS = (image) => {
    if (!this.isOSSelected(image)) {
      // assume selecting different os
      // override version
      this.overrideVersion = null;
    }
    this.selectedOS = image;
  };
  ...
  this.getCurrentOSVersion = (image) => {
    if (this.isOSSelected(image)) {
  ...
  this.isOSSelected = (image) => {
    if (!this.selectedOS) {
      return false;
    }

    return image.$$hashKey === this.selectedOS.$$hashKey;
  };
