...
module.exports = function(
  ...
  /**
   * last selected board label
   */
  this.lastSelectedBoardLabel = null;

  /**
   * @summary get last selected board
   * @function
   * @public
   *
   * @returns {Object} board object from getBoardList
   *
   * @example
   * ImageSelectorController.getLastSelectedBoard();
   */
  this.getLastSelectedBoard = () => {
    return ImageSelectorService.lastSelectedBoard;
  };

  /**
   * @summary set selected board
   * @function
   * @public
   *
   ...
   *
   * @example
   * ImageSelectorController.setLastSelectedBoard();
   */
  this.setLastSelectedBoard = (board) => {
    this.lastSelectedBoardLabel = board.name;
    ImageSelectorService.lastSelectedBoard = board;
  };

  this.getSupportedExtension = () => {
    const exts = this.supportedExtensions.concat();
    _.map(exts, (ext, idx) => {
  ...
  this.getBoardList = () => {
    ...
    $http.get(AppConfigService.BOARDS_DATA + '?d=' + new Date().getTime())
      .then((payload) => {
        ...
        // initially, check for last selected board
        if (this.getLastSelectedBoard()) {
          let foundLastSelectedBoard = false;
          for (const i in this.boards) {
            if (this.boards[i].name === this.getLastSelectedBoard().name) {

              // select last board
              this.getBoardOSes(this.boards[i]);
              foundLastSelectedBoard = true;
              break;
            }
          }
          if (foundLastSelectedBoard === false) {
            this.setLastSelectedBoard(null);

            // last board not found, select the first board
  ...
  this.getOSLogo = (imagePath) => {
    if (typeof imagePath !== 'string') {
      throw new Error('imagePath is not a string');
    }

    if (imagePath.includes('://')) {
      // imagePath is an absolute path
      return imagePath;
    }
  ...
  this.closeModal = (option) => {
    ...
    if (option) {
      if (option.hasCurrentSelectedImageFile()) {
        ...
        let selectedVersion = option.selectedOS.versions[0];

        if (option.overrideVersion) {
          selectedVersion = _.find(option.selectedOS.versions, (item) => {
            return item.version === option.overrideVersion;
          });
        }
