...
const app = angular.module('Etcher', [
  ...
  require('./components/instruction-modal/instruction-modal'),
...
app.run((SettingsModel) => {
  // at initial stage, downloadPath is not set
  if (!fs.existsSync(SettingsModel.getAll().downloadPath)) {
    const defaulImagePath = path.join(electron.remote.app.getPath('userData'), 'downloadedImage');

    fs.mkdir(defaulImagePath, '0777', function(e) {
      if (e === null || e.code === 'EEXIST') {
        SettingsModel.set('downloadPath', defaulImagePath);
      } else {
        console.log('Failed to create default download directory!');
        console.log(e);
      }
    });
  }
...
app.run((AnalyticsService, OSWindowProgressService, FlashStateModel) => {
  Store.subscribe(() => {
    ...
    AnalyticsService.logDebug([
      ...
      `${flashState.percentage}% at ${units.bytesToSize(flashState.speed)} /s`,
...
app.run(($window, WarningModalService, ErrorService, FlashStateModel, OSDialogService) => {
  ...
  $window.addEventListener('beforeunload', (event) => {
    ...
    return OSDialogService.showWarning({
      ...
      title: 'Are you sure you want to close PINE64 Installer?',
...
app.controller('FooterController', function(FlashStateModel) {
  this.shouldSettingBeDisabled = () => {
    return FlashStateModel.isFlashing();
  };
});
