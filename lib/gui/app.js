...
app.run((SettingsModel) => {
  ...
  if (!SettingsModel.getAll().downloadPath) {
    console.log('Download path is not found. Use user\'s download path');

    // use user's download folder
    SettingsModel.set('downloadPath', electron.remote.app.getPath('downloads'));
  }
});
