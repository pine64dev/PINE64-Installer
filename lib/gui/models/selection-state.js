...
SelectionStateModel.service('SelectionStateModel', function(DrivesModel) {
   ...
   * @public
   *
   * @param {Object} data - OS data
   *
   * @example
   * SelectionStateModel.setOnlineOSImages({
   *  name: 'Ubuntu Mate Desktop',
   *  thumb: 'ubuntu.png',
   *  version: '20161215'
   *  images: [
   *    {
   *      url: 'http://url.to.the.fileserver/os.img.gz',
   *      checksum: '11C2675947EA1ECA94907719252483BD',
   *      checksumType: 'md5',
   *      recommendedDriveSize: 8000000
   *    }
   *   ]
   *  });
   */
  this.setOSImages = (data) => {
    // @NOTE: to be continue by CS

    // temporarily set image
    Store.dispatch({
      type: Store.Actions.SELECT_IMAGE,
      data: {
        logo: 'http://192.168.0.181/pine64_etcher/' + data.thumb,
        name: data.name,
        path: '_temp_path',
        size: -1
      }
  ...
  /**
   * @summary Get OS Build Version
   * @function
   * @public
   *
   * @returns {Object} build number
   *
   * @example
   * const version = SelectionStateModel.getOSVersion();
   */
  this.getOSVersion = () => {
    // @TODO: get from Store.selection.os.version
    if (this.getImagePath() === '_temp_path') {
      return '20170318';
    }
