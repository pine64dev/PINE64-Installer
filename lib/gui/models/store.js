  ...
  switch (action.type) {
    ...
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      if (action.data.length === 1) {
        ...
        if (_.isEmpty(os)) {
          // constraints based on local image
          isDriveOk = _.every([
          ...
          // constraints based on os
          if (drive.recommendedImage) {
            isDriveOk = _.every([
              // valid drive
              !constraints.isDriveLocked(drive),
              constraints.isDriveLargeEnough(drive, {
                size: drive.recommendedImage.recommendedDriveSize
              }),
              !constraints.isSourceDrive(drive, {
                path: '/'
              }),
              !constraints.isSystemDrive(drive)
            ]);
