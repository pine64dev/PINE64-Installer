...
const storeReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ACTIONS.SELECT_OS: {
      ...
      return _.attempt(() => {
        //console.log('recommendedDriveSize:', selectedDrive.recommendedDriveSize);
        if (selectedDrive && !getRecommendedImage(selectedDrive.toJS(), action.data)) {
        ...
        return state;
      }).setIn(['selection', 'os'], Immutable.fromJS(action.data));
    }
    ...
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      if (os) {
        _.forEach(action.data, (drive) => {
          drive['recommendedImage'] = getRecommendedImage(drive, os)
        })
      }
      else
      {
        image = state.getIn(['selection', 'image'], Immutable.fromJS({})).toJS();
      }
    ...
    case ACTIONS.REMOVE_OS: {
      return state.deleteIn([ 'selection', 'os' ]);
    }
