...
module.exports = function(
  ...
  /**
   * @summary Convert seconds to readable time.
   * @function
   * @public
   *
   * @param {Number} seconds = seconds
   *
   * @returns {String} Time in hh[h]m[m] or m[m]ss[s] or s[s] format
   *
   * @example
   * FlashController.getDisplayETA();
   */
  this.getDisplayETA = (seconds) => {
    const sec = parseInt(seconds, 0);
    let displayTime;

    if (sec >= 3600) {
      displayTime = moment(0).utc().seconds(sec).format('h[h]mm[m]');
    } else if (sec >= 60) {
      displayTime = moment(0).utc().seconds(sec).format('m[m]ss[s]');
    } else {
      displayTime = moment(0).utc().seconds(sec).format('s[s]');
    }
    return displayTime;
  };

  /**
   * @summary Convert bytes to size.
   * @function
   * @public
   *
   * @param {Number} bytes = bytes
   *
   * @returns {String} bytes either in Bytes, KB, MB, GB
   *
   * @example
   * FlashController.getBytesToSize();
   */
  this.getBytesToSize = (bytes) => {
