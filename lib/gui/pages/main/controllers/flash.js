...
module.exports = function(
   ...
   *
   * @returns {String} Time in hh[h]m[m] or hh[h]m[m] format
   *
   * @example
  ...
  this.getDisplayETA = (seconds) => {
    ...
    if (sec >= 3600) {
      displayTime = moment().utc().seconds(sec).format('h[h]mm[m]');
    } else if (sec >= 60) {
      displayTime = moment().utc().seconds(sec).format('m[m]ss[s]');
    } else {
      displayTime = moment().utc().seconds(sec).format('s[s]');
    }
    return displayTime;
  };
