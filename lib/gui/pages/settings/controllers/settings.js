...
module.exports = function(WarningModalService, SettingsModel, AnalyticsService) {
   ...
   *
   * @description
   * If warningOption is given, it should be an object having `description` and `confirmationLabel`;
   * these will be used to present a user confirmation modal before enabling the setting.
   *
   * @param {String} setting - setting key
   * @param {Object} warningOptions - (optional) WarningModalService options
   * @returns {Undefined}
   *
   * @example
   ...
   * });
   */
  this.toggle = (setting, warningOptions) => {

    const value = this.currentData[setting];
    const dangerous = Boolean(warningOptions);
    ...
    WarningModalService.display(warningOptions).then((userAccepted) => {
      if (userAccepted) {
