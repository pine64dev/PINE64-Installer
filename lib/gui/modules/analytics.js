...
const makeFlatStartCaseObject = require('../../shared/utils').makeFlatStartCaseObject;
...
analytics.service('AnalyticsService', function($log, $window, $mixpanel, SettingsModel) {
  ...
  this.logEvent = (message, data) => {
    ...
    const flatStartCaseData = makeFlatStartCaseObject(data);
    if (SettingsModel.get('errorReporting') && isRunningInAsar()) {
      $mixpanel.track(message, flatStartCaseData);
    }

    if (flatStartCaseData) {
      message += ` (${JSON.stringify(flatStartCaseData)})`;
    }
